...
function TabsContent({ children }: { children: React.ReactNode }) {
  const { activeContentId, registerContent, unregisterContent } =
    useTabsContext();
...
function TabsNavigate() {
  ...
  const handlePrev = () => {
    const currentIndex = tabs.findIndex(
      (tab) => tab.contentId === activeContentId,
    );
...
function ModalContent({ children }: { children: React.ReactNode }) {
  ...
  return isOpen ? <div className="modal-content">{children}</div> : null;
}
...
function RootAccordion({ children }: { children: React.ReactNode }) {
  ...
  const registerContent = useCallback((contentId: string) => {
    setItems((prev) => {
      const alreadyRegistered = prev.find(
...
function AccordionTrigger({ children }: { children: React.ReactNode }) {
  const { registerTrigger, items, setActiveContentId, activeContentId } =
    useAccordionContext();
  ...
  return (
    <div className="app">
      <button onClick={() => setVisible(!visible)}>
        Hide/Show Tab 2: {visible ? "Hide" : "Show"}
      </button>
      <Tabs>
        <Tabs.List>
